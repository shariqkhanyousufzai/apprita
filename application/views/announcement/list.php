            ...
            <div class="card card-custom gutter-b">
                <div class="card-header flex-wrap border-0 pt-6 pb-0">
                    <div class="card-title">
                        <h3 class="card-label">Announcment Listing
                            <span class="d-block text-muted pt-2 font-size-sm">Announcment listing</span>
                        </h3>
                    </div>
                    <div class="card-toolbar">
                        <!--begin::Button-->
                        <a href="<?php echo base_url('announcement/create'); ?>" class="btn btn-light-primary btn-pill font-weight-bolder">
                                            ...
                                            <div class="d-flex align-items-center">
                                                ...
                                                <select class="form-control" id="kt_datatable_search_type">
                                                    <option value="">All</option>
                                                       ...
                                                       ?>
                                                       <option value="<?=$getAnnouncmentCategorie->name?>"><?=$getAnnouncmentCategorie->name?></option>
                                                       <?php
                                                       }
                                                       ?>
                                             </select>
...
$(document).ready(function(){
    $(document).on('click','.read_accouncement',function(){
        var getAnnouncementId = $(this).data('id');
        $.ajax({
            type : 'post',
            url :"<?=base_url('announcement/getannouncementbyid')?>",
            data : {
                getAnnouncementId : getAnnouncementId
            },
            success:function(data){
                var res = JSON.parse(data);
                $('.announcement_msg').html(res.body);
